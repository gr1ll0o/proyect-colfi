"""
Conversor de archivos a BITS y viceversa
--
v1.3
...
root.title("colFi. - Conversor de archivos-bytes")
...
def file_to_bits():
    """
    Convierte un archivo a una secuencia de bits (8 bits por byte) 
    """
    try:
        file = filedialog.askopenfilename(title="Selecciona un archivo")
        log(f"Archivo seleccionado: {file}")
...
def bits_to_file():
    """
    Reconstruye un archivo desde una secuencia de bits
    """
    try:
        txt_path = filedialog.askopenfilename(title="Selecciona un texto de cadena de bytes (.txt)", filetypes=(("Archivo texto", "*.txt"),))
    ...
    try:
        ...
        log("Creando archivo...")
        extension = simpledialog.askstring('Extension del archivo', "Escriba la extension del archivo SIN el punto.")
        output_path = os.path.join(path, f"{filename}.{extension}")
...
def start_conversion():
    ...
    if (modo.get() == 0):
        log("--- Archivo a Bytes ---")
        log("Esperando por un archivo...")
        file_to_bits()
    elif (modo.get() == 1):
        log("--- Bytes a Archivos ---")
